...
def update_deployment_spec(params):
    ...
    if "version" in params:
        spec["spec"]["template"]["metadata"]["labels"]["version"] = params["version"]

    env_list = spec["spec"]["template"]["spec"]["containers"][0]["env"]
    env_dict = {var["name"]: var["value"] for var in env_list}
    ...
    env_updated = [{"name": key, "value": value} for key, value in env_dict.items()]
    spec["spec"]["template"]["spec"]["containers"][0]["env"] = env_updated
...
def delete_serving_deployment(params):
    ...
    if name in [deployment.metadata.name for deployment in api_response.items]:
        api_response = api_client.delete_namespaced_deployment(name, namespace, del_opts)
    else:
...
def get_pods(params):
    ...
    def match_seldon_deployment(pod):
        ...
        return env["TRAINING_ID"] == training_id
...
def get_deployment_status(params):
    ...
    if status in ["Ready", "PROGRESSING", "Progressing"]:
        status = "Available"
...
def get_serving_deployment_url(params):
    # "http://${PUBLIC_IP}:${SELDON_AMBASSADOR_PORT}/seldon/${deployment_name}/api/v0.1/predictions"
    ip = params["public_ip"]
    name = get_deployment_name(params)
    url = "http://%s/predict" % (ip)
    return url
...
def run_safe(params, method):
    try:
        ...
        if method in ("POST", "PATCH", "PUT"):
            ...
            delete_result = delete_serving_deployment(params)
            result = {
                "status": "Success"
            }
        else:
